...
class Game:
    ...
    def reset_world(self):
        ...
        self.cars = [Vehicle(*self.city.random_open_position(120, 25), color=random.choice(car_colors)) for _ in range(55)]
        self.traffic = [Vehicle(*self.city.random_open_position(120, 25), color=random.choice(car_colors), traffic=True) for _ in range(75)]
        self.traffic.extend(Vehicle(*self.city.random_open_position(120, 25, "Civic Center"), traffic=True, model="police") for _ in range(12))
        self.enemies = [Enemy(*self.city.random_open_position(120, 18, random.choice(["Slums", "Industrial", "Market Mile"]))) for _ in range(30)]
        self.police = [Police(*self.city.random_open_position(120, 18, random.choice(["Civic Center", "Downtown", "Market Mile"]))) for _ in range(34)]
        self.police.extend(Police(*self.city.random_open_position(120, 18, "Market Mile")) for _ in range(8))
    ...
    def load_game(self):
        ...
        self.missions.index = min(data.get("mission_index", 0), len(self.missions.missions))
    ...
    def interior_events(self, e):
        ...
        elif e.key in (pygame.K_1, pygame.K_2, pygame.K_3, pygame.K_4, pygame.K_5) and items:
    ...
    def trigger_bank_robbery(self):
        ...
        if not bank:
            return
        payout = random.randint(650, 1050)
        self.player.money += payout
        self.last_bank_robbed = True
        self.bank_heist_timer = 45
        self.wanted.level = max(self.wanted.level, 4)
        self.wanted.timer = max(self.wanted.timer, 45.0)
        ...
        for _ in range(5):
            ...
            car = Vehicle(cx + math.cos(angle) * radius, cy + math.sin(angle) * radius, traffic=True, model="police")
    ...
    def buy_interior_item(self):
        ...
        elif item.startswith("Medkit") or item.startswith("Full Heal"):
            self.player.health = self.player.max_health
        ...
        elif item in ("Pistol", "Shotgun", "Rifle"):
            self.player.unlock_weapon(item)
        elif item == "Repair Current Car" and self.player.in_car:
            self.player.in_car.health = 180
        elif item == "Performance Tune" and self.player.in_car:
            self.player.in_car.max_speed += 80
            self.player.in_car.accel += 70
    ...
    def draw_hud(self):
        ...
        controls = self.font_small.render("E car/interact  Z building  M map  H hospital  G garage  B bank  F fullscreen  1-5 weapons", True, (190, 190, 190))
    ...
    def draw_full_map(self):
        ...
        title = self.font_large.render("CITY MAP", True, YELLOW)
        ...
        legend = "B bank   H hospital   P police   G gun shop   R garage   S gas   M/Esc close"
    ...
    def draw_mission_log(self):
        ...
        title = self.font_large.render("MISSION LOG", True, YELLOW)
    ...
    def draw_gunshop(self):
        ...
        panel = pygame.Rect(SCREEN_W // 2 - 230, 500, 460, 155)
    ...
    def draw_interior(self):
        ...
        items = self.interior_items()
        if items:
            panel = pygame.Rect(SCREEN_W // 2 - 250, SCREEN_H - 205, 500, 145)
